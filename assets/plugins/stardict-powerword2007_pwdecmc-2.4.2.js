    ...
    if (event == 'ready') {
        ...
        var handlePhonetic = function(ItemDiv) {
            ...
            for (var i = 0; i < phonetics.length; i++) {
                ...
                e.outerHTML = '<span style="color:green;">' + fixPhonetic(e.innerHTML) + '</span>';
            }
        }
                    ...
                    for (var item = e.firstElementChild; item != null; item = item.nextElementSibling) {
                        switch (item.ElementName) {
                            case 'icibaNodeClass16':
                                item.setAttribute("style", "margin:1px;font-weight:900;background-color:#dde;height:24px;line-height:24px;");
                            ...
                            case 'icibaNodeClass14':
                                ...
                                for (var next = item.nextElementSibling; next != null && next.ElementName == 'icibaNodeClass14';) {
                                    ExampleHandler(next);
                                ...
                                item.innerHTML = '<span style="margin:8px 0px 5px 0.5em; padding:0 40px 0 0; color:#333333;font-weight:700; background:url(img/examples1.png) no-repeat; background-position:right center; display:inline-block;">例句</span>' + '<div style ="border: 1px solid #366; border-radius: 3px; padding:5px;">' + html + '</div>';
                    ...
                    case 'icibaNodeClass7':
                        ...
                        for (var item = e.firstElementChild; item != null; item = item.nextElementSibling) {
                            switch (item.ElementName) {
                                case 'icibaNodeClass16':
                                    var word = getWordText(item);
                                    html = '<span style="font-weight:700; display:inline-block; color:#630;">' + word + '</span>';
                                    break;
                                case 'icibaNodeClass12':
                                    html = html + '<span style="padding:3px;">' + item.innerHTML + '</span>';
                                    break;
                                case 'icibaNodeClass8':
                                    html = html + '&nbsp;&nbsp;[<span style="color:green;">' + item.innerText + '</span>]';
                                    break;
                                case 'icibaNodeClass18':
                                    html = html + '<span style="padding:0px 6px 0px 6px;font-weight:700;display:inline-block;">' + item.innerHTML + '</span>';
                            }
                        }
                        e.innerHTML = html;
                        break;
                        ...
                        for (var item = e.firstElementChild; item != null; item = item.nextElementSibling) {
                            switch (item.ElementName) {
                                case 'icibaNodeClass16':
                                    var word = getWordText(item);
                                    if (word != '') {
                                        html = '<span style="font-weight:700; display:inline-block; padding-right:1.5em;color:#630;" >' + word + '</span>';
                                    }
                                    break;
                                case 'icibaNodeClass12':
                                    html = html + '<span style="padding:3px;">' + item.innerHTML + '</span>';
                                ...
                                case 'icibaNodeClass8':
                                    ...
                                    break;
                                case 'icibaNodeClass18':
                                    html = html + '<span style="padding:2px;">' + item.innerHTML + '</span>';
                            }
                        }
                        e.innerHTML = html;
                }
